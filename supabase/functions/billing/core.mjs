 ...
 return async req=>{
  ...
  try{
   ...
   if(input.action==='checkout'){
    ...
    // New base + extra-seat commercial rule requires a two-item subscription.
    // Never create a subscription using the superseded all-seats Team price.
    if(input.plan==='team')fail('team_pricing_pending',409);
