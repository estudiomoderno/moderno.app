...
window.SettingsUI=(()=>{
  ...
  function values(){
    ...
    if(section==='estudio')return {name:A.name||'',...Object.fromEntries(['type','location','country','about','taxId','web','instagram','facebook','email','phone','address','address2','city','postcode','province','logo'].map(k=>[k,S[k]??'']))};
    if(section==='impuestos')return {...Object.fromEntries(['country','currency','taxName','vat','irpf','iban'].map(k=>[k,A[k]??''])),retention:!!state.irpf,margin:state.defMargin??0,fc:serie(true),ps:serie(false),...Object.fromEntries(['name','nif','addr','city','country','phone','iban'].map(k=>['emitter_'+k,state.emitter?.[k]??'']))};
  ...
  function view(){
    ...
    if(owner!==sessionKey()){metadata=null;profileAttempt='';start();}else if(!draft)start();
    ...
    return `<div class="settings-page"><header><h1>${title}</h1><p>Ajustes de Moderno.app</p></header><form id="settings-form"><fieldset class="settings-fieldset" ${busy?'disabled':''}>${content()}</fieldset>${save?`<div class="settings-actions"><span class="settings-status" id="settings-status" role="status" aria-live="polite" data-error="${error}">${E(message||'Los cambios se aplican al guardar.')}</span><button class="btn btn-ghost" type="button" data-settings-action="reset" ${busy?'disabled':''}>Descartar cambios</button><button class="btn btn-dark" type="submit" ${busy||(['cuenta','notificaciones'].includes(section)&&!metadata)?'disabled':''}>${busy?'Guardando…':'Guardar cambios'}</button></div>`:''}</form></div>`;
  }
  function discardThen(fn){if(busy){toast('Espera a que termine el guardado.');return;}if(dirty()){askConfirm('Tienes cambios sin guardar en Ajustes. ¿Quieres descartarlos?','Descartar',()=>{draft=null;fn();});return;}fn();}
  function open(k='cuenta'){if(!allowed(k))return;discardThen(()=>{if(!active())previous=state.view;section=k;draft=null;metadata=null;profileAttempt='';go('ajustes');document.body.classList.remove('sbopen');document.getElementById('settings-switch').open=false;loadPersonal();if(k==='equipo')loadInvitations();});}
  ...
  document.addEventListener('input',e=>{if(e.target.dataset.setting&&draft&&!busy){const el=e.target;draft[el.dataset.setting]=el.type==='checkbox'?el.checked:el.value;status('Cambios sin guardar.');}});
  document.addEventListener('change',e=>{if(e.target.dataset.photo)image(e.target.files[0],e.target.dataset.photo);if(e.target.dataset.setting&&draft&&!busy){const el=e.target;draft[el.dataset.setting]=el.type==='checkbox'?el.checked:el.value;status('Cambios sin guardar.');}});
