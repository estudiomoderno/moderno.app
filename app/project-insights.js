...
function projectAlerts(p,includeQuotes=true){const m=ModernoProjectInsights.metrics(p),bits=[],qs=includeQuotes?projectPendingQuotes(p):[];if(qs.length)bits.push(qs.length+' presupuesto'+(qs.length===1?' pendiente':'s pendientes'));if(m.overdue)bits.push(m.overdue+' tarea'+(m.overdue===1?' vencida':'s vencidas'));if(m.stale)bits.push('Sin actividad registrada en 14 días');return bits.length?'<span class="project-alert">'+bits.join(' · ')+'</span>':''}
