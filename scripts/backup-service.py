...
def run():
    print('Stage: configuration',flush=True)
    ...
    with tempfile.TemporaryDirectory(prefix='moderno-service-') as directory:
        ...
        with ftplib.FTP_TLS(context=ssl.create_default_context(),timeout=60) as ftp:
            print('Stage: FTPS connection',flush=True)
            ftp.connect(os.environ['FTP_HOST'],21)
            print('Stage: FTPS authentication',flush=True)
            ftp.login(os.environ['FTP_USER'],os.environ['FTP_PASSWORD']);ftp.prot_p()
            print('Stage: source directory',flush=True)
            ftp.cwd(os.environ['FTP_SERVER_DIR'])
            print('Stage: source inventory',flush=True)
            before=inventory(ftp)
            print('Stage: scope validation',flush=True)
            if not before or not any(x['path']=='api/enviar-invitacion.php' for x in before) or sum(x['size'] for x in before)>50_000_000:
                raise ValueError('scope')
            print('Stage: source download',flush=True)
            for item in before:
            ...
            print('Stage: source stability',flush=True)
        ...
        print('Stage: private Drive upload',flush=True)
        rclone('copy',str(root),destination,'--immutable')
        print('Stage: private Drive verification',flush=True)
        ...
        print('Stage: completion marker',flush=True)
