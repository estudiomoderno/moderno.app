...
test('completed tasks are excluded for current and legacy status formats',async()=>{const tasks=[{title:'Visible',col:'todo'},{title:'Terminada',col:'listo'},{title:'Legacy',col:'done'},{title:'Marcada',done:true}].map((t,i)=>({...t,id:i,due:'2026-09-10'}));const f=fixture({data:{proyectos:[{id:1,tasks}]}});const text=await(await f.run('a'.repeat(48))).text();assert.match(text,/Visible/);assert.doesNotMatch(text,/Terminada|Legacy|Marcada/);});
