...
const studyCode="function drawSidebarStudy(){\n const el=document.getElementById('sidebarStudy');if(!el)return;\n const keys=visibleWsOrder(),key=keys.includes(state.brandFilter)?state.brandFilter:keys[0];\n const name=key?(BRAND[key]?.name||'Mi estudio'):'Mi estudio',E=ModernoDaily.escape;\n const logo=key&&state.logos?.[key];\n el.innerHTML=`<details class=\"study-dropdown\"><summary aria-label=\"Men\u00fa del estudio: ${E(name)}\"><span class=\"study-avatar\">${logo?`<img ${fileImageAttrs(logo)} alt=\"\">`:E(name.slice(0,1))}</span><span class=\"study-name\">${E(name)}</span><span class=\"study-chevron\" aria-hidden=\"true\">\u2303<br>\u2304</span></summary><div class=\"study-popup\">\n <div class=\"study-options\">${keys.map((k,i)=>`<button type=\"button\" data-study-index=\"${i}\">${E(BRAND[k]?.name||'Mi estudio')}</button>`).join('')}</div>\n <button type=\"button\" data-study-action=\"settings\">${ico('m_settings')||ico('tools')}Ajustes</button>\n ${_accessRole==='admin'?`<button type=\"button\" data-study-action=\"invite\">${ico('m_invitar')||ico('contactos')}Invitar al equipo</button>`:''}\n <button type=\"button\" data-study-action=\"notifications\"><svg aria-hidden=\"true\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.7\"><path d=\"M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4\"/></svg>Notificaciones</button>\n <button type=\"button\" data-study-action=\"logout\">${ico('atras')}Cerrar sesi\u00f3n</button>\n </div></details>`;\n el.querySelectorAll('[data-study-index]').forEach(b=>b.onclick=()=>{state.brandFilter=keys[Number(b.dataset.studyIndex)];drawBrandSwitch();render();});\n el.querySelectorAll('[data-study-action]').forEach(b=>b.onclick=()=>{el.querySelector('details').open=false;const a=b.dataset.studyAction;if(a==='settings')profileMenu();if(a==='invite'&&_accessRole==='admin')inviteUserModal(-1);if(a==='notifications')notifModal();if(a==='logout')sbLogout();});\n el.onkeydown=e=>{if(e.key==='Escape'){el.querySelector('details').open=false;el.querySelector('summary').focus();}};\n}\n";
